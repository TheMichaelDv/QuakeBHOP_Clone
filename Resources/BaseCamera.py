...
class Camera:
    ...
    def _update_yaw_and_pitch(self) -> None:
        ...
        self.dir = vector3.normalise(front)
        self._dir = Vector3([vector3.normalise(front)[0], 0, vector3.normalise(front)[2]])
        self.right = vector3.cross(self.dir, self._up)
        self.up = vector3.cross(self.right, self.dir)
...
class KeyboardCamera(Camera):
    ...
    def matrix(self) -> numpy.ndarray:
        ...
        if self._zdir == NEGATIVE:
            self.position += self._dir * self._velocity * t
        elif self._zdir == POSITIVE:
            self.position -= self._dir * self._velocity * t

        # Y Movement
        if self._ydir == POSITIVE:
            self.position += Vector3([0,0.5,0]) * self._velocity * t
